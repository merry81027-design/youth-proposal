...
def load_data():
    data = pd.DataFrame({
        ...
        "提案原因": ["手機統一鎖起來，反而放大了我們的數位焦慮，且無法學習自我控制。", "7:30 到校讓多數學生睡眠不足，早自習考試的學習效益極低。"],
        "提案內容": ["推動【數位健康契約】與榮譽置物櫃制度，上課交機，下課開放。", "推動【彈性 8:30 到校制】，廢除早自習，開放圖書館安置提早到校學生。"],
        
        "角色1_學生": ["我們需要查資料、安排補習時間，沒收手機等於切斷了我們的數位自主權...", "睡眠不足導致我們上課根本無法集中，早自習考試的效益極低..."],
        "角色2_教師": ["上課偷滑手機嚴重干擾教學節奏，且引發網路霸凌事件，班級經營難度極高...", "早自習是安定班級心情的重要時刻，若取消，第一節課學生往往心浮氣躁..."],
        "角色3_家長": ["雖然怕他們近視，但如果發生緊急事件（如地震），聯絡不到孩子我會非常焦慮...", "雙薪家庭早上必須配合上班時間送小孩，若延後上學，家長接送會有極大困難..."],
        "角色4_校長": ["我必須兼顧校園安全與學校聲譽。若開放手機導致偷拍或作弊事件，學校會面臨極大的輿論與家長壓力...", "延後到校牽涉到校車班次的全面調度，以及鄰近交通尖峰時刻的壅塞問題，不能只看單一班級的意願..."],
        "角色5_部長": ["中央法規賦予學校因地制宜的管理權，但政策必須符合《兒童及少年福利與權益保障法》，保障學生的表意權...", "配合 108 課綱精神，我們鼓勵學校將早自習還給學生自主規劃，但同時要確保城鄉差距與弱勢學生的照顧..."],
        "角色6_教授": ["大學端看重的是『自我調節能力』。高中把學生管得太死，上大學沒人管就全面失控，這不是我們要的未來人才...", "只會早起考試的機器人無法適應高等教育。我們希望看到學生有精神地參與討論，而不是在課堂上補眠..."]
    })
    return data
    ...
    with cols[index % 2]:
     # 下面這些程式碼的最前面，必須對齊得一模一樣！
      ...
      # 切出兩個小區塊，比例為 1:2
        ...
        # 右邊的大區塊放提案原因與內容
        with text_col:
            st.markdown(f"**❓ 提案原因：** {row['提案原因']}")
            st.markdown(f"**💡 提案內容：** {row['提案內容']}")
...
components.iframe("https://padlet.com/embed/iipny64x1ou2s45t", height=400, scrolling=True)
st.divider()
